    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        ...
        activity?.let {
            ...
            if (pending != null) {
                ...
                var task = when (call.method) {
                    CREATE_USER_METHOD -> auth.startActivityForSignInWithProvider(it, provider).addOnSuccessListener {
                        result.success("")
                    ...
                    LINK_USER_METHOD -> {
                        ...
                        if (user == null) {
                            FirebaseAuthOAuthPluginError.PluginError("").toResult(result)
                        }

                        user!!.startActivityForLinkWithProvider(it, provider).addOnSuccessListener {
                            result.success(null)
                            return@addOnSuccessListener
                        }
                    }
                    else -> com.google.android.gms.tasks.Tasks.forCanceled()
                }
